...
pair<list<int>::iterator, int> find_sequence() {
    list<int>::iterator start = beads.begin(), end;
    int same_num = beads.front();
    int same_cnt = 1;
    for (end = next(beads.begin()); end != beads.end(); ++end) {
        if (*end == 0) break;
...
void bomb_beads() {
    pair<list<int>::iterator, int> seq_info = find_sequence();
    list<int>::iterator start_iter = seq_info.first;
    int num = seq_info.second;

    while (num != -1) {
        bomb_bead(start_iter, num);
        seq_info = find_sequence();
        start_iter = seq_info.first; num = seq_info.second;
    }
